...
public class MyDiningListAdapter extends RecyclerView.Adapter<MyDiningListAdapter.ViewHolder> {
    ...
    public static class ViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        public ViewHolder(View itemView,int ViewType,Context c) {                 // Creating ViewHolder Constructor with View and viewType As a parameter
            ...
            else if(ViewType == TYPE_SUBTITLE) {
                textView = (TextView) itemView.findViewById(R.id.rowText); // Creating TextView object with the id of textView from item_row.xml
                imageView = (ImageView) itemView.findViewById(R.id.rowIcon);// Creating ImageView object with the id of ImageView from item_row.xml
                Holderid = -1;                                               // setting holder id as 1 as the object being populated are of type item row
    ...
    MyDiningListAdapter(String Titles[],int Icons[],String Subtitles[],String Name,String Email, int Profile,Context passedContext){ // MyDiningListAdapter Constructor with titles and icons parameter
    ...
    @Override
    public MyDiningListAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {

        if (viewType == TYPE_ITEM) {
            View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.dining_item_row,parent,false); //Inflating the layout
            ...
            View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.dining_header,parent,false); //Inflating the layout
            ...
            View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.subtitle, parent, false); //Inflating the layout

            ViewHolder vhSubtitle = new ViewHolder(v, viewType, context); //Creating ViewHolder and passing the object of type view

            return vhSubtitle; //returning the object created

        } else if (viewType == TYPE_LINE) {

            View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.line, parent, false); //Inflating the layout

            ViewHolder vhLine = new ViewHolder(v, viewType, context); //Creating ViewHolder and passing the object of type view

            return vhLine; //returning the object created
        }
    ...
    @Override
    public void onBindViewHolder(MyDiningListAdapter.ViewHolder holder, int position) {
        if(holder.Holderid ==1) {                              // as the list view is going to be called after the header view so we decrement the
            ...
            // position by -1 and pass it to the holder while setting the text and image
            holder.textView.setText(mNavSubTitles[position - 7]); // Setting the Text with the array of our Titles
        }
